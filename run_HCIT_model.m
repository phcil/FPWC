...
fracBW = 0.1 % fractional bandwidth of control
Nlambda = 3 % number of wavelength samples across control band
lambda_vec = linspace(lambda0 - lambda0*fracBW/2, lambda0 + lambda0*fracBW/2, Nlambda) % vector of control wavelengths
Dpup = 48e-3; % meters
Ddm = Dpup; % meters
z_dm1_dm2 = 1; % meters
...
if (num_dms==2) || (which_dm==1)        % DM1, compute Jacobian
    ...
    for q = 1:Nact^2,   
        ...
        [Etemp1,~] = HCIT_model(Ein,I00,SP0,DM1V,DM2V,VtoH1,VtoH2,Ddm,Nact,...
          sampling,lambda0,lambda,z_dm1_dm2,fl_M1,fl_M2,fl_M3,Dpup,IP_OWA,0,1,1,1,genInfCubeFlag,infCube);
...
if (num_dms==2) || (which_dm==2)        % DM2 (after pupil), compute Jacobian
    ...
    for q = 1:Nact^2,   
        ...
        [Etemp2,~] = HCIT_model(Ein,I00,SP0,DM1V,DM2V,VtoH1,VtoH2,Ddm,Nact,...
          sampling,lambda0,lambda,z_dm1_dm2,fl_M1,fl_M2,fl_M3,Dpup,IP_OWA,0,1,1,2,genInfCubeFlag,infCube);
...
%         DMcalc = -(EyeM/mu + M)\EimProj;
    ...
    sampling,lambda0,lambda,z_dm1_dm2,fl_M1,fl_M2,fl_M3,Dpup,IP_OWA,abFlag,0,num_dms,which_dm,genInfCubeFlag,infCube);
%     Eout = -conj(Eout);
    Im = abs(Eout).^2;
